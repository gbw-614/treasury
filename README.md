...
The verifier does not derive expected label wording from a COLA class code or
ask reviewers to reconstruct missing application facts. Each case must supply
the exact visible values that should appear on its artwork; a `null` value means
that field is intentionally not compared. See the
[verification input contract](docs/VERIFICATION_INPUT_CONTRACT.md).

The intended deployment target is a Docker image running the web application,
Python API, Tesseract OCR, and a persistent SQLite/artwork volume on one EC2
instance.
