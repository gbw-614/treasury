  ...
  useEffect(() => {
    ...
    const loadCapabilities = async () => {
      try {
        ...
        if (!cancelled) {
          setReaderCapabilities({ visionAvailable: false, availableReaderModes: ["ocr"], catalogConfigured: false });
          setReaderMode("ocr");
          setReaderModeDraft("ocr");
        }
      }
    ...
    <main className="app-shell verify-shell">
      ...
      {activeTab === "queue" ? (
        <WorkQueue
          ...
          catalogConfigured={readerCapabilities.catalogConfigured}
          onBatchImport={() => openQueueEntry("batch")}
          onCatalogImport={() => { if (readerCapabilities.catalogConfigured) openQueueEntry("catalog"); }}
