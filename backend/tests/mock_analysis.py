...
def build_mock_analysis(
    ...
    *,
    ...
    if request.schema_version == "verification-request-v2":
        transcript = "\n".join(field.raw_text for field in active_fields)
        v2_results: list[AdditionalRuleResult] = []
        for check in request.checks:
            definition = get_field_definition(check.field_id)
            if not check.required:
                v2_results.append(AdditionalRuleResult(
                    field_id=definition.id, label=definition.name,
                    match_mode=definition.check_type, automated_status=AutomatedStatus.DOES_NOT_APPLY,
                    expected_value=check.expected_value, detected_value=None, evidence_quote=None,
                    reason_code="not_required", explanation="This field is not required for this case.",
                    requires_human_review=False,
                ))
                continue
            evaluation = evaluate_check(definition, transcript, check.expected_value)
            v2_results.append(AdditionalRuleResult(
                field_id=definition.id, label=definition.name,
                match_mode=definition.check_type,
                automated_status=AutomatedStatus.MATCHES if evaluation.matched else AutomatedStatus.REVIEW,
                expected_value=check.expected_value, detected_value=evaluation.detected_value,
                evidence_quote=evaluation.evidence_quote, reason_code=evaluation.reason_code,
                explanation=evaluation.explanation, requires_human_review=not evaluation.matched,
            ))
        additional_rules = tuple(v2_results)
        rules = ()
    else:
