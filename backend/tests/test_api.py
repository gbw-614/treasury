...
def test_capabilities_hide_vision_without_a_server_key(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv(s3_catalog.CATALOG_ENV, raising=False)
...
def test_capabilities_expose_configured_catalog(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(s3_catalog.CATALOG_ENV, "https://example.test/catalog/manifest.json")

    response = client.get("/api/v1/capabilities")

    assert response.status_code == 200
    assert response.json()["catalogConfigured"] is True
