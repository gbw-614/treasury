...
def catalog_is_configured() -> bool:
    """Return whether this deployment exposes a public catalog.

    This deliberately checks only for an explicit setting.  A malformed or
    temporarily unavailable configured catalog should remain visible and
    report its actionable error in the import view.
    """
    return bool(os.getenv(CATALOG_ENV, "").strip())
